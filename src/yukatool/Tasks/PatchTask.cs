	...
	class PatchTask : Task {
		...
		protected override void Execute() {
			...
			if(flags.Has("init")) {
				...
				string patchFilePath = Path.ChangeExtension(patchDirectory, Constants.patchLogExtension);
				...
				// patch archive
				if(targetBasePath.EndsWith(Constants.archiveExtension, StringComparison.OrdinalIgnoreCase)) {
				...
				// patch file
				else if(targetBasePath.EndsWith(Constants.patchLogExtension, StringComparison.OrdinalIgnoreCase)) {
					...
					foreach(string filePath in localFiles) {
						...
						if(history.Contains(fileName)) {
							...
							for(int i = 1; i <= 5; i++) {
								...
								string archivePath = Path.Combine(gameDirectory, archiveName + '.' + Constants.archiveExtension);
